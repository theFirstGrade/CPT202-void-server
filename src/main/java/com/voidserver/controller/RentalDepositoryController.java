...
public class RentalDepositoryController {
    ...
    public Result rentalDepository(Integer currentPage) {
        ...
        productDepository = rentalDepositoryService.getRentalProductDepository1(new Page<>(currentPage, 9));
        ...
        if (searchCate.equals("全部") || searchCate.equals("All")) {
            if (searchName.equals("")) {
                ...
                if (searchAddress.equals("全部") || searchAddress.equals("All")) {
                    ...
                    productDepository = rentalDepositoryService.getRentalProductDepository2(new Page<>(currentPage, 9), searchAddress);

                }
            } else {
                if (searchAddress.equals("全部") || searchAddress.equals("All")) {
                    productDepository = rentalDepositoryService.getRentalProductDepository7(new Page<>(currentPage, 9), searchName);
                } else {
                    productDepository = rentalDepositoryService.getRentalProductDepository3(new Page<>(currentPage, 9), searchAddress, searchName);
                }
            }
        } else {
            if (searchName.equals("")) {
                if (searchAddress.equals("全部") || searchAddress.equals("All")) {
                    productDepository = rentalDepositoryService.getRentalProductDepository4(new Page<>(currentPage, 9), searchCate);
                } else {
                    productDepository = rentalDepositoryService.getRentalProductDepository5(new Page<>(currentPage, 9), searchCate, searchAddress);
                }
            } else {
                if (searchAddress.equals("全部") || searchAddress.equals("All")) {
                    productDepository = rentalDepositoryService.getRentalProductDepository8(new Page<>(currentPage, 9), searchCate, searchName);
                }
                productDepository = rentalDepositoryService.getRentalProductDepository6(new Page<>(currentPage, 9), searchCate, searchAddress, searchName);
            }
        }
